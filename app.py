...
# --- MODEL CHAINS (STRICTLY RESTORED) ---
MODEL_CHAINS = {
    "GEMINI": [
...
def call_ai_text(model_id, history, image_data=None, deep_think=False):
    ...
    payload = { 
        "contents": history,
        "system_instruction": {
            "parts": [{"text": "You are Omni-Chat, a helpful, intelligent assistant."}]
        }
    }
...
def live_socket(ws):
    ...
    # Use the first model in the Native Audio chain
...
def home():
    ...
    <body>
        ...
        <script>
            ...
            function openImgModal() { document.getElementById("imgModal").style.display = "flex"; }
            ...
            function addMsg(content, type, isHtml=false, isLive=false) {
                ...
                if(typeof content === 'string') {
                    ...
                    if (content.tagName === 'IMG') d.appendChild(createImgContainer(content));
            ...
            async function sendText() {
                ...
                if (imageRegex.test(t)) {
                    addMsg(t, "user"); txtIn.value = ""; txtIn.style.height = "48px";
                    addLoading();
                    try {
                    ...
                    } catch(e) { removeLoading(); addMsg("Image Gen Error: " + e, "ai"); }
                    return; 
                }
            ...
            // --- LIVE CALL ---
            let mediaRecorder, ws, audioContext, audioQueue=[], isPlaying=false;
            ...
            function playPCM(b64) {
                ...
                audioQueue.push(buf); schedulePlayback();
            }

            function schedulePlayback() {
                if(isPlaying || audioQueue.length===0) return;
                isPlaying=true; let src=audioContext.createBufferSource(); src.buffer=audioQueue.shift();
                src.connect(audioContext.destination); src.onended=()=>{ isPlaying=false; schedulePlayback(); }; src.start();
            }
